...
function ChangeDummyVehicleClass (int classNum) 
{
	...
	if(rxPC.GetTeamNum() == TEAM_GDI)
	{
		MaterialInstanceConstant(DummyVehicle.Mesh.GetMaterial(0)).SetVectorParameterValue('Camo_Colour', NewColour);
		MaterialInstanceConstant(DummyVehicle.Mesh.GetMaterial(0)).SetTextureParameterValue('Camo', Texture2D'RenX_AssetBase.Vehicle.T_Camo_Nod_Desert');
	}
}
